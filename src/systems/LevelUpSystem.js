...
class LevelUpSystem {
  ...
  _showAnnounce() {
    ...
    this._add(s.add.text(W / 2, H / 2 - 12, 'LEVEL UP', {
      fontFamily: GAME_FONT, fontSize: '14px', color: '#ffdd44', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));

    // Level number
    this._add(s.add.text(W / 2, H / 2 + 8, `Level ${GameState.player.level}`, {
      fontFamily: GAME_FONT, fontSize: '9px', color: '#ccaa33', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));
    ...
    const prompt = s.add.text(W / 2, H / 2 + 48, '- Press any button -', {
      fontFamily: GAME_FONT, fontSize: '6px', color: '#888866', padding: FONT_PAD,
  ...
  _showChooseMenu() {
    ...
    this._add(s.add.text(W / 2, 24, `LEVEL ${GameState.player.level}`, {
      fontFamily: GAME_FONT, fontSize: '10px', color: '#ffdd44', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));

    this._add(s.add.text(W / 2, 40, 'Choose your reward', {
      fontFamily: GAME_FONT, fontSize: '7px', color: '#aa9944', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));
    ...
    for (let i = 0; i < LEVELUP_CHOICES.length; i++) {
      ...
      const label = s.add.text(W / 2, y, choice.label, {
        fontFamily: GAME_FONT, fontSize: '9px', color: '#ffffff', padding: FONT_PAD,
      }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51);

      const desc = s.add.text(W / 2, y + 14, choice.desc, {
        fontFamily: GAME_FONT, fontSize: '6px', color: '#888888', padding: FONT_PAD,
      }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51);
    ...
    this._add(s.add.text(W / 2, H - 16, 'Up/Down to select, Attack to choose', {
      fontFamily: GAME_FONT, fontSize: '5px', color: '#555544', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));
  ...
  _showConfirm(index) {
    ...
    this._add(s.add.text(W / 2, H / 2 - 30, choice.label, {
      fontFamily: GAME_FONT, fontSize: '12px', color: '#ffdd44', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));
    ...
    this._add(s.add.text(W / 2, H / 2 + 22, 'Attack = Confirm', {
      fontFamily: GAME_FONT, fontSize: '7px', color: '#88cc88', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));

    this._add(s.add.text(W / 2, H / 2 + 38, 'Jump = Go Back', {
      fontFamily: GAME_FONT, fontSize: '7px', color: '#cc8888', padding: FONT_PAD,
    }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(51));
  }
